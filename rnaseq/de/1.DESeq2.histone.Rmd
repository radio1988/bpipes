...
volcanoplot <- function (res, 
    ...
    # all
    with(res_, plot(log2FoldChange, -log10(pvalue), pch=20, main=main, col="grey80",cex=.5,
                    ylim=c(0,50), ...))
    # small P
    with(subset(res_, padj<sigthresh ), 
         points(log2FoldChange, -log10(pvalue), pch=20, col="grey40",cex=.5, ...))
    # large FC
    with(subset(res_, abs(log2FoldChange)>lfcthresh), 
         points(log2FoldChange, -log10(pvalue), pch=20, col="grey60",cex=.5, ...))
    with(subset(res_, padj<sigthresh & abs(log2FoldChange)>lfcthresh), 
         points(log2FoldChange, -log10(pvalue), pch=20, col="black",cex=.5, ...))
    if (labelsig) {
        require(calibrate)
        with(subset(res_, padj<sigthresh & abs(log2FoldChange)>lfcthresh), 
...
colnames(count) <- paste(colnames(tpm),"COUNT", sep = ":")
