...
# Notes:
- Good FastQC: ~25-40M fragments/lib, 150bp PE
- Unstranded
- Unique mapping rate ~93%, Good
- FeatureCount assignment rate: ~60%, a bit lower than normal
...
```{r design}
type <- factor(rep(c("DM4", "DM5", "DM_G", "O4", "O5", "O_G"), each=3))
type
# 
# batch <- factor(rep(c("1", "2", "3"),6))
# batch
# 
...
```{r}
pcaData <- plotPCA(vsd, intgroup=c("type", "type"), returnData=TRUE)
...
## One Big Model (DM-G_vs_DM5)
```{r}
#resultsNames(dds)
name <- "DM-G_vs_DM5"
contrast <- c('type', 'DM_G', 'DM5')
res <- lfcShrink(dds, contrast = contrast, type = 'ashr')
res2 <- results(dds, contrast = contrast)
process_deseq_res(res = res, res2=res2, name = name, anno = anno, norm_exp = tpm)
```

## One Big Model (DM-G_vs_O_G)
```{r}
#resultsNames(dds)
name <- "DM-G_vs_O-G"
contrast <- c('type', 'DM_G', 'O_G')
res <- lfcShrink(dds, contrast = contrast, type = 'ashr')
res2 <- results(dds, contrast = contrast)
process_deseq_res(res = res, res2=res2, name = name, anno = anno, norm_exp = tpm)
```

## One Big Model (O-G_vs_O4)
```{r}
#resultsNames(dds)
name <- "O-G_vs_O4"
contrast <- c('type', 'O_G', 'O4')
res <- lfcShrink(dds, contrast = contrast, type = 'ashr')
res2 <- results(dds, contrast = contrast)
process_deseq_res(res = res, res2=res2, name = name, anno = anno, norm_exp = tpm)
```

## One Big Model (O-G_vs_O5)
```{r}
#resultsNames(dds)
name <- "O-G_vs_O5"
contrast <- c('type', 'O_G', 'O5')
res <- lfcShrink(dds, contrast = contrast, type = 'ashr')
res2 <- results(dds, contrast = contrast)
process_deseq_res(res = res, res2=res2, name = name, anno = anno, norm_exp = tpm)
```
