...
df <- readxl::read_xlsx( '../DESeq2/T13_vs_WT.deseq2.sig.FDR.0.05.LFC.1.xlsx', na='-')  # na term important
...
head(num_df)
write.csv(num_df, paste(name,"sig.zscore.csv", sep = "."), row.names = T)
```
...
#header = data.frame(type = rep(c('DMSO', "Fasudil", "KD025" ), each=2))  # params
header <- data.frame(type = c("T13", "T13", 
                              "T18", "T18", 
                              "T21", "T21", "T21", "T21", "T21", "T21","T21",
                              "WT",   "WT",   "WT",   "WT",   "WT", "WT" ))
...
### Hide gene names
```{r, fig.height = 6, fig.width = 7, fofig.align = "center"}
Heatmap(num_df, 
...
### Show gene names
```{r, fig.height = 21, fig.width = 8, fofig.align = "center"}
Heatmap(num_df,
        column_title = outname,
        row_title = paste(num_sig, 'DEG'),
        name = 'Z-score',
