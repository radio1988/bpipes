...
df <- readxl::read_xlsx( '../DESeq2/RSL_vs_DMSO.deseq2.sig.FDR.0.05.LFC.0.xlsx', na='-')  # na term important
...
header = data.frame(type = rep(c('DMSO', "RSL"), each=2))  # params

ha = HeatmapAnnotation(df = header,  
                       col = list(type = c("DMSO" =  "green", 
                                           'RSL' = 'orange'
                                           )))
```
...
### Hide gene names
```{r, fig.height = 8, fig.width = 5, fofig.align = "center"}
Heatmap(num_df, 
...
### Show gene names
```{r, fig.height = 21, fig.width = 6, fofig.align = "center"}
Heatmap(num_df, 
        column_title = outname,
        row_title = paste(num_sig, 'DEG'),
        name = 'Z-score',
