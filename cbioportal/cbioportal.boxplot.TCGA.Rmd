...
res <- apply(data.frame(my_genes), 1, test, expr=expr, cliclass=cliclass)
print(res)
...
box_jitter_plot_multiple_groups <- function(expr, cliclass, 
    ...
    # leven's test
    leveneTest(expr[,my_genes[1]] ~ expr[, cliclass], data = expr)
 
    #print(compare_means(c(PSTK, SCLY, SEPHS2, SEPSECS, SLC7A11) ~ PR_STATUS_BY_IHC, data = expr))
    levene_res <- leveneTest(y = expr[,'SEPHS2'], group =  expr[,'TNBC'], data = expr)
