...
box_jitter_plot <- function(expr, cliclass, 
    ...
    ggboxplot(expr, x = cliclass,
              ...
              ylab = "Expression (Z-score)", # can adjust
              xlab = F,
...
box_jitter_plot_anova(expr, "AJCC_PATHOLOGIC_TUMOR_STAGE", c("I", "IA",  "IB",   
                                                       "II", "IIA", "IIB", 
                                                       "III", "IIIA", "IIIB", "IIIC", 
                                                       "IV", "X"))

violin_box_plot_anova(expr, "AJCC_PATHOLOGIC_TUMOR_STAGE", c("I", "IA",  "IB",
                                                       "II", "IIA", "IIB",
                                                       "III", "IIIA", "IIIB", "IIIC",
                                                       "IV", "X"))
```
...
```
```{r}
violin_box_plot_anova <- function(expr, cliclass, labels=  c("Positive", "Negative"), xlab_angle=0, ylab_height=-2.5){
    # Violin + Boxplot
    ggviolin(expr, x = cliclass,
              select = labels,
              order = labels,
              xlab = F,
              y = c("PSTK", "SCLY", "SEPHS2", "SEPSECS", "SLC7A11"),
              combine = T,
              color = cliclass, palette = "jco",
              ylab = "Expression (RPKM)",
              add = "boxplot") +
        #stat_compare_means(method = "anova", label.y = ylab_height) +  # global annova pvalue
...
box_jitter_plot_multiple_groups <- function(expr, cliclass, 
    ...
    pairs <- expand.grid(labels, labels)
    pairs <- as.vector(t(pairs))
    i <- seq(1,length(pairs),2)
    get_pair <- function(i, pairs){pairs[i:(i+1)]}
    sapply(i, get_pair, pairs)
    ...
    ggboxplot(expr, x = cliclass,
        ...
        theme(
            ...
            axis.text.x=element_blank(), 
            axis.ticks.x=element_blank()) + # removed xlabs
        stat_compare_means(comparisons =  list(labels[1:2], labels[1:3], labels[1:4],
                                               labels[2:3], labels[2:4], 
                                               labels[3:4]), 
