...
rule multiQC_sorted_reads:
    ...
    output:
        "results/sorted_reads_bam_qc/multiqc_report.html",
    resources:
        mem_mb=lambda wildcards, attempt: attempt * 3000
    threads:
        1
    ...
    shell:
        """
        multiqc -f {input.stats} {input.idxstats} {input.flagstat} -o results/sorted_reads_bam_qc/ &> {log}
        """
...
rule multiQC_clean_reads:
    ...
    output:
        "results/clean_reads_bam_qc/multiqc_report.html"
    resources:
        mem_mb=lambda wildcards, attempt: attempt * 3000
    threads:
        1
    ...
    shell:
        """
        multiqc -f {input.stats} {input.idxstats} {input.flagstat} -o results/clean_reads_bam_qc/ &> {log}
